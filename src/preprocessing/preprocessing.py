...
STEP_SIZE_SECONDS = 0.25   # 75% overlap
...
# # Duration and step of each processing window in seconds
# WINDOW_SIZE_SECONDS = 2.0  # 1-second windows
# STEP_SIZE_SECONDS = 0.25   # 75% overlap
...
#         if data.shape[0] < MIN_REQUIRED_SAMPLES:
#             print(f"Skipping {file.name} — not enough samples")
#             continue
...
#         segments = window_data(filtered, WINDOW_SIZE, STEP_SIZE)

#         # Normalize each window independently
#         segments = np.array([
#             (seg - np.mean(seg, axis=0)) / (np.std(seg, axis=0) + 1e-6)
#             for seg in segments
#         ])

#         if len(segments) == 0:
#             print(f"Skipping {file.name} — no valid segments")
#             continue
...
#         print(f"{file.name}: {len(segments)} segments extracted")
...
#     print("Final shape before saving:", all_segments.shape)
#     np.save(save_dir / 'preprocessed_data.npy', all_segments)
#     np.save(save_dir / 'labels.npy', labels_array)
#     print(f"Saved preprocessed dataset to: {save_dir}")
